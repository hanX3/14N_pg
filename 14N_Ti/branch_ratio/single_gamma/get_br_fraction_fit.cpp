...
void get_br(TString str, int dis, Double_t rebin)
{
  double p0 = -0.746728;
  double p1 = 0.511636;
  ...
  //
  TFile *fi_mc = TFile::Open(TString::Format("dis_%dcm/mc_%s.root",dis,str.Data()));
  if(fi_mc->IsZombie()){
    cout << "can not open " << TString::Format("dis_%dcm/mc_%s.root",dis,str.Data()) << endl;
    return;
  }
  ...
  leg->AddEntry(h_exp, "h_exp");
  for(int i=0;i<7;i++){
    leg->AddEntry(h_sim_scale[i], TString::Format("gamma_%dkeV", gamma[i]).Data());
  }
  leg->AddEntry(mc_add, "mc add");
...
//
void get_mc_hist(TString str, int dis)
{
  gROOT->SetBatch(1);
  ...
  ifstream fi;
  fi.open(TString::Format("dis_%dcm/filename_%s.dat",dis,str.Data()));

  if(!fi){
    cout << "can not open " << TString::Format("dis_%dcm/filename_%s.dat",dis,str.Data()) << endl;
    return ;
  }

  TFile *fi_out = new TFile(TString::Format("dis_%dcm/mc_%s.root",dis,str.Data()), "recreate");
  TFile *fi_sim[7];
  TTree *tr_sim[7];
  TH1D *h_sim[7];
  int i = 0;
