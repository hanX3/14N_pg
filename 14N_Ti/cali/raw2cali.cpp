...
void raw2cali(TString target, TString energy)
{
  double p0 = -0.746728;
  double p1 = 0.511636;
  ...
  ifstream fi;
  fi.open(TString::Format("../../../data/14N_Ti/%s/14N_%skeV.txt", target.Data(), energy.Data()).Data());
  if(!fi){
  ...
  TFile *file_out = new TFile(TString::Format("./rootfile/%s_%skeV_cali.root", target.Data(), energy.Data()).Data(), "recreate");
  file_out->cd();
  h->Write();
  h_cali->Write();
  tr->Write();
