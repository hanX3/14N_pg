...
void raw2cali_special(TString target, TString energy)
{
  double p0 = -0.746728;
  double p1 = 0.511636;
  ...
  TFile *file_out = new TFile(TString::Format("./rootfile/%s_%skeV_cali_2.root", target.Data(), energy.Data()).Data(), "recreate");
  file_out->cd();
  h->Write();
  h_cali->Write();
  tr->Write();
